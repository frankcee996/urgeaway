...
public class AppLockPlugin extends Plugin {
    ...
    // as it runs, so the app can show — without any logcat/adb access —
    // whether the service is genuinely alive and receiving window events,
    // versus the OS Settings toggle being on but the service actually
    // dead/never bound (a common real-device failure mode, e.g. a
    // manufacturer's battery optimizer killing it in the background).
    @PluginMethod
    public void getDiagnostics(PluginCall call) {
        JSONObject diag = AppLockStore.readDiagnostics(getContext());
        JSObject ret = new JSObject();
        ret.put("accessibilityEnabled", isServiceEnabled());
        ret.put("serviceConnectedAt", diag.optLong("serviceConnectedAt", 0));
        ret.put("lastEventAt", diag.optLong("lastEventAt", 0));
        ret.put("lastEventPackage", diag.opt("lastEventPackage"));
        ret.put("lastBlockAt", diag.optLong("lastBlockAt", 0));
        ret.put("lastBlockPackage", diag.opt("lastBlockPackage"));
        ret.put("lastBlockActionResult", diag.optBoolean("lastBlockActionResult", false));
        ret.put("now", System.currentTimeMillis());
        call.resolve(ret);
    }
